...
- It writes PyFluent code that executes inside the running solver via `ion exec`
...
- Each step is verified by querying `ion inspect last.result` — the agent **reads the structured output before deciding what to do next**
...
[ion](https://github.com/svd-ai-lab/ion) is a lightweight runtime that connects AI agents to physics solvers. For Fluent, it works like this:
...
- **`ion exec`** — send a Python snippet that runs inside the live solver, with `solver` object already injected
- **`ion inspect`** — query session state: which step just ran, did it succeed, what was the result
- **`ion disconnect`** — cleanly shut down the Fluent process
...
Claude sends a snippet via `ion exec`. The `solver` object is already injected into the namespace — no `import`, no `launch_fluent()`:

```python
solver.settings.file.read_case(file_name=r"E:\ion\ion\mixing_elbow.msh.h5")
...
```
Reading from DESKTOP-623UBP1:"E:\ion\ion\mixing_elbow.msh.h5" in NODE0 mode ...
  Reading mesh ...
...
The agent confirms via `ion inspect last.result`:
...
> **`ion screenshot`** after reading mesh:
>
...
> **`ion screenshot`** after mesh check:
>
...
> **`ion screenshot`** after zone discovery:
>
...
> **`ion screenshot`** after physics setup:
>
...
> **`ion screenshot`** after material assignment:
>
...
> **`ion screenshot`** after boundary conditions:
>
...
> **`ion screenshot`** — residuals converging:
>
...
> **`ion screenshot`** — results extracted:
>
...
3. `ion exec` sends it to the running Fluent process
4. Fluent executes it — the GUI updates live
5. `ion inspect last.result` returns structured JSON: ok, stdout, result
...
```bash
ion connect --solver fluent --mode meshing --ui-mode gui --processors 2
```
...
The key insight: **PyFluent is the automation interface, and ion manages the session lifecycle.** Once you have a persistent session and structured result inspection, the agent has the same access to Fluent that a human has through the GUI — plus the ability to programmatically check every result and decide what to do next.

| Without ion | With ion |
|---|---|
