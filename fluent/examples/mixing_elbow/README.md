...
```bash
# 1. Start ion server on a machine with Fluent installed (needs GUI session)
ion serve
...
# 3. Run each step
ion exec --file snippets/00_read_case.py        --label read-case
ion exec --file snippets/01_mesh_check.py       --label mesh-check
ion exec --file snippets/02_diagnose_zones.py   --label diagnose-zones
ion exec --file snippets/03_setup_physics.py    --label setup-physics
ion exec --file snippets/04_setup_material.py   --label setup-material
ion exec --file snippets/05_setup_bcs.py        --label setup-bcs
ion exec --file snippets/06_hybrid_init.py      --label hybrid-init
ion exec --file snippets/07_run_iterations.py   --label run-iterations
ion exec --file snippets/08_extract_outlet_temp.py --label extract-results

# 4. Check results after each step
ion inspect last.result
